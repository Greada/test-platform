...
> 前置:Lesson 1.5 已完成 Dockerfile + compose 动手实验,产出 `Dockerfile.new` + `docker-compose.learn.yml`
...
> ⚠️ 本课用 `docker-compose.learn.yml`(指向 `Dockerfile.new`),不用生产版 `docker-compose.yml`(指向原版 `Dockerfile`)。保持学习链一致:Jenkinsfile.new → docker-compose.learn.yml → Dockerfile.new。
...
│   ├── backend/Dockerfile.new       ← 学习 Dockerfile(L1.5 产出)
│   ├── frontend/Dockerfile
│   ├── frontend/Dockerfile.new      ← 学习 Dockerfile(L1.5 产出)
│   └── Jenkinsfile.new
└── (其他仓库文件)
```
...
## 三、最终 Jenkinsfile.new 结构(Lesson 2 完成时)
