...
Find every server-side handler. Use `codegraph_explore` with `src/fn`, `src/routes`, `src/queues`, `ipcMain` to map handler structure. If `.codegraph/` not initialized, fall back to `grep` + `read` + `glob`.

By convention in TanStack Start:
...
Find the project's existing auth helpers (e.g., `requireUser()`, `requireAdmin()`, `getCurrentUser()`, `assertCanAccess(post, user)`). Use `codegraph_search` to find auth-related symbols. If `.codegraph/` not initialized, fall back to `grep`. Use those — do not invent new ones.
