...
2. Walk imports of project-local components (ignore `node_modules`, design-system primitives like shadcn `ui/*` unless the user names them). Use `codegraph_explore` to trace imports. If `.codegraph/` not initialized, fall back to `read` + `grep`.
